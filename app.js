...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const activateSystem = async () => {
    try {
      ...
      // 1. Explicitly request microphone access
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      console.log("Microphone access granted via getUserMedia");
      
      // 2. Request Notification Permissions for Mobile/Desktop
      if ("Notification" in window) {
        const result = await Notification.requestPermission();
        console.log("Notification permission state:", result);
      }
      
      // 3. Stop the temporary stream
      stream.getTracks().forEach(track => track.stop());

      // 4. Start the Speech Recognition engine
      if (recognition) {
        recognition.start();
